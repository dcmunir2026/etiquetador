/**
 * Dev seed — wipes all data and re-inserts the canonical set:
 *   - 1 superadmin (Marta, password marta1234)
 *   - 6 intensity scales (with their levels)
 *   - 11 dimensions (with their default values)
 *   - 4 projects (Marta as projectadmin on each)
 *   - 4 taxonomies (with their N:M dimension links)
 *   - Project ↔ Taxonomy assignments for the demo projects
 *
 * Idempotent: re-running on an already-seeded DB clears and re-inserts.
 * Safe for dev. Do NOT run in production.
 *
 * Usage: `pnpm --filter @etiquetador/db seed`
 */
...
async function main() {
  ...
  await db.delete(taxonomyDimensions);
  await db.delete(projectMembers);
  await db.delete(taxonomies);
  await db.delete(dimensionValues);
  await db.delete(dimensions);
  await db.delete(intensityLevels);
  await db.delete(intensityScales);
  ...
  const [marta] = await db
    ...
    .returning();
  if (!marta) throw new Error('Failed to insert marta');
  ...
  for (const s of scales) {
    const [row] = await db
      ...
      .values({ name: s.name, kind: s.kind, isCustom: false, createdBy: marta.id })
      .returning();
    if (!row) throw new Error(`Failed to insert scale ${s.name}`);
  ...
  for (const d of dimData) {
    ...
    const [row] = await db
      ...
      .returning();
    if (!row) throw new Error(`Failed to insert dimension ${d.name}`);
  ...
  for (const p of projectData) {
    const [row] = await db
      ...
      .returning();
    if (!row) throw new Error(`Failed to insert project ${p.name}`);
  ...
  const taxData = [
    {
      name: 'Sesgos sociodemográficos',
      shortDesc: 'Sesgos contra grupos protegidos: odio, género, raza, religión, demografía.',
      color: 'rose',
      dims: ['Sesgo de odio', 'Género', 'Raza / etnia', 'Religión', 'Sesgo demográfico'],
    },
    {
      name: 'Calidad periodística',
      shortDesc: 'Rigor factual, datos, fuentes, verificación. Métricas formales de calidad.',
      color: 'amber',
      dims: ['Sesgo estadístico', 'Incoherencia factual'],
    },
    {
      name: 'Aspectos formales del discurso',
      shortDesc: 'Estilo, retórica, carga emocional, figuras literarias. Cómo se dice, no qué se dice.',
      color: 'cyan',
      dims: ['Carácter tendencioso', 'Semiótica', 'Emotividad'],
    },
    {
      name: 'Toxicidad y discurso dañino',
      shortDesc: 'Insultos, vulgaridad, agresividad. No incluye odio estructural.',
      color: 'violet',
      dims: ['Toxicidad'],
    },
  ];
  ...
  for (const t of taxData) {
    ...
    const [row] = await db
      ...
      .returning();
    if (!row) throw new Error(`Failed to insert taxonomy ${t.name}`);
    taxIds[t.name] = row.id;
    for (let i = 0; i < t.dims.length; i++) {
      const dimName = t.dims[i]!;
